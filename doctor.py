...
class UiDoctorWindow(QMainWindow):
    def __init__(self, parent=None):
        super(UiDoctorWindow, self).__init__(parent)
        self.CNX = None

    def setupUi(self):
        self.setObjectName("DoctorWindow")
        ...
        self.doctor_table = QtWidgets.QTableWidget(self.centralwidget)
        self.doctor_table.setGeometry(QtCore.QRect(40, 60, 1021, 551))
        self.doctor_table.setLayoutDirection(QtCore.Qt.LeftToRight)
        self.doctor_table.setObjectName("doctor_table")
        self.doctor_table.setColumnCount(8)
        ...
        item = QtWidgets.QTableWidgetItem()
        self.doctor_table.setHorizontalHeaderItem(6, item)
        item = QtWidgets.QTableWidgetItem()
        self.doctor_table.setHorizontalHeaderItem(7, item)
    ...
    def retranslateUi(self):
        ...
        self.setWindowTitle(_translate("DoctorWindow", "Doctor Manage"))
        self.label.setText(_translate("DoctorWindow", "Doctors"))
        item = self.doctor_table.horizontalHeaderItem(0)
        item.setText(_translate("DoctorWindow", "ID"))
        item = self.doctor_table.horizontalHeaderItem(1)
        item.setText(_translate("DoctorWindow", "Name"))
        item = self.doctor_table.horizontalHeaderItem(2)
        item.setText(_translate("DoctorWindow", "Phone"))
        item = self.doctor_table.horizontalHeaderItem(3)
        item.setText(_translate("DoctorWindow", "Email"))
        item = self.doctor_table.horizontalHeaderItem(4)
        item.setText(_translate("DoctorWindow", "Address"))
        item = self.doctor_table.horizontalHeaderItem(5)
        item.setText(_translate("DoctorWindow", "Hospital"))
        item = self.doctor_table.horizontalHeaderItem(6)
        item.setText(_translate("DoctorWindow", "Edit"))
        item = self.doctor_table.horizontalHeaderItem(7)
        item.setText(_translate("DoctorWindow", "Delete"))
        self.import_button.setText(_translate("DoctorWindow", "Import"))
        self.export_button.setText(_translate("DoctorWindow", "Export"))
        self.search_box.setTitle(_translate("DoctorWindow", "Search doctor"))
        self.search_button.setText(_translate("DoctorWindow", "Search doctor"))
        self.search_name.setText(_translate("DoctorWindow", "Name:"))
        self.tab_widget.setTabText(self.tab_widget.indexOf(self.search), _translate("DoctorWindow", "Search"))
        self.add_button.setText(_translate("DoctorWindow", "Add"))
        self.add_name.setText(_translate("DoctorWindow", "Name:"))
        self.add_phone.setText(_translate("DoctorWindow", "Phone:"))
        self.add_address.setText(_translate("DoctorWindow", "Address:"))
        self.add_hospital_id.setText(_translate("DoctorWindow", "Hospital ID:"))
        self.add_email.setText(_translate("DoctorWindow", "Email:"))
        self.tab_widget.setTabText(self.tab_widget.indexOf(self.add_new), _translate("DoctorWindow", "Add New"))

    def show_data_table(self, data):
        self.doctor_table.clearContents()
        self.doctor_table.setRowCount(0)
        len_data = len(data)
        for i in range(len_data):
            self.doctor_table.insertRow(i)
            item = QtWidgets.QTableWidgetItem(str(data[i][0]))
            item.setFlags(item.flags() ^ QtCore.Qt.ItemIsEditable)
            self.doctor_table.setItem(i, 0, item)
            # self.doctor_table.setItem(i, 0, QtWidgets.QTableWidgetItem(str(data[i][0])))  # id
            self.doctor_table.setItem(i, 1, QtWidgets.QTableWidgetItem(str(data[i][1])))  # name
            self.doctor_table.setItem(i, 2, QtWidgets.QTableWidgetItem(str(data[i][2])))  # phone
            self.doctor_table.setItem(i, 3, QtWidgets.QTableWidgetItem(str(data[i][3])))  # email
            self.doctor_table.setItem(i, 4, QtWidgets.QTableWidgetItem(str(data[i][4])))  # address
            self.doctor_table.setItem(i, 5, QtWidgets.QTableWidgetItem(str(data[i][5])))  # hospital id

            self.edit_button = QtWidgets.QPushButton(self)
            self.edit_button.clicked.connect(self.edit_button_click)
            self.edit_button.setText("Edit")
            self.doctor_table.setCellWidget(i, 6, self.edit_button)

            self.delete_button = QtWidgets.QPushButton(self)
            self.delete_button.clicked.connect(self.delete_button_click)
            self.delete_button.setText("Delete")
            self.doctor_table.setCellWidget(i, 7, self.delete_button)
    ...
    def init_data(self):
        ...
        if self.connect_db_status is False:
            return
        self.data = mysql_db.search_all_name(self.cursor, 'doctor')
    ...
    def search_button_click(self):
        ...
        self.data = mysql_db.search_all_name(self.cursor, 'doctor', search_name)
        self.show_data_table(self.data)

    def delete_button_click(self):
        button = QtWidgets.qApp.focusWidget()
        index = self.doctor_table.indexAt(button.pos())
        # if index.isValid():
        #     print(index.row(), index.column())
        table_model = self.doctor_table.model()
        ...
        if delete_confirm == QMessageBox.Yes:
            delete_success = mysql_db.delete_row_by_id(self.CNX, self.cursor, 'doctor', id)
    ...
    def edit_button_click(self):
        ...
        #     print(index.row(), index.column())
        table_model = self.doctor_table.model()
        num_cols = self.doctor_table.columnCount()
        update_data = []
        for i in range(num_cols - 2):
            data_index = table_model.index(index.row(), i)
            update_data.append(table_model.data(data_index))
        print(update_data)
        msg = common.check_doctor_input(self.cursor, update_data[1], update_data[2], update_data[3],
        ...
        if msg != '':
            ...
            if edit_confirm == QMessageBox.Yes:
                edit_success = mysql_db.update_doctor(self.CNX, self.cursor, update_data[0], update_data[1],
    ...
    def add_button_click(self):
        ...
        msg = common.check_doctor_input(self.cursor, add_name, add_phone, add_email, add_address, add_hospital_id)
        if msg != '':
            QMessageBox.information(self, 'Input data invalid', msg, QMessageBox.Close)
            return
        else:
            insert_success = mysql_db.add_doctor(self.CNX, self.cursor, add_name, add_phone,
    ...
    def import_button_click(self):
        ...
        column_count = self.doctor_table.columnCount()
        column_label = []
        for i in range(column_count - 2):
            column_label.append(self.doctor_table.horizontalHeaderItem(i).text())
        # print(import_data)
        ...
        for row in import_data:
            try:
                mysql_db.add_doctor(self.CNX, self.cursor, row[1], row[2], row[3], row[4], row[5])
            except:
    ...
    def export_button_click(self):
        ...
        for i in range(column_count - 2):
            column_label.append(self.doctor_table.horizontalHeaderItem(i).text())
        file_path = common.save_excel('doctor', column_label, self.data, EXPORT_PATH)
